...
fn solution(c: *aoc.Context) !void {
    ...
    const memory = try parseMemory(c.allocator(), groups.items[1]);
    var computer = try Computer.init(c.allocator(), a, memory);
    answer.part1([]const u8, "7,0,3,1,2,6,3,7,1", try part1(c.allocator(), &computer));
