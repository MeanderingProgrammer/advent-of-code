...
class Program:
    ...
    def __init__(self, memory, to_input, debug=False):
        self.memory, self.to_input, self.debug, self.outputs = memory, to_input, debug, []
    ...
    def run(self):
        ...
        while pointer < len(self.memory):
            instruction = Instruction(self.memory[pointer:])
            if self.debug:
                print(instruction)
            if instruction.halt():
                break
            result = instruction.process(self)
            pointer = pointer + len(instruction) if result is None else result
